...
def accuracy_calculator(test_x, test_y, classifier):
    accuracy = 0
...
def split_dataset(dataset, split_ratio):
    split_index = int((split_ratio / 100) * len(dataset))
    train_set = dataset[split_index:]
    test_set = dataset[:split_index]
    return train_set, test_set


def remove_feature(dataset, index):
    return [row[:index] + row[index+1:] for row in dataset]


def split_train_test(train_set, test_set):
...
if __name__ == '__main__':
    ...
    train_x, train_y, test_x, test_y = split_train_test(train_set, test_set)

    classifier = DecisionTreeClassifier(criterion='gini', random_state=0)
    classifier.fit(train_x, train_y)

    importances = list(classifier.feature_importances_)
    index = importances.index(min(importances))

    dataset_updated = remove_feature(dataset, index)
    train_set_updated, test_set_updated = split_dataset(dataset_updated, x)

    train_x_updated, train_y_updated, test_x_updated, test_y_updated = split_train_test(train_set_updated, test_set_updated)

    standard_scaler = StandardScaler()
    standard_scaler.fit(train_x_updated)
    min_max_scaler = MinMaxScaler()
    min_max_scaler.fit(train_x_updated)

    train_x_standard_scaled = standard_scaler.transform(train_x_updated)
    test_x_standard_scaled = standard_scaler.transform(test_x_updated)

    train_x_minmax_scaled = min_max_scaler.transform(train_x_updated)
    test_x_minmax_scaled = min_max_scaler.transform(test_x_updated)

    neural_network_classifier = MLPClassifier(15, activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)

    neural_network_classifier.fit(train_x_standard_scaled, train_y_updated)
    accuracy_standard = accuracy_calculator(test_x_standard_scaled, test_y_updated, neural_network_classifier)
    print(f'Tocnost so StandardScaler: {accuracy_standard}')

    neural_network_classifier.fit(train_x_minmax_scaled, train_y_updated)
    accuracy_minmax = accuracy_calculator(test_x_minmax_scaled, test_y_updated, neural_network_classifier)
