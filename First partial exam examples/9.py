...
class BoardGame(Problem):
    ...
    def successors(self, state):
        ...
        for i in range(0, len(fields), n):
            row = []
            for j in range(n):
                row.append(fields[i + j])
            matrix.append(row)

        changes = [(0, 0), (0, 1), (0, -1), (-1, 0), (1, 0)]
        ...
        for i in range(len(matrix)):
            for j in range(len(matrix[i])):
                ...
                for change in changes:
                    new_x, new_y = i + change[0], j + change[1]
                    if self.check_valid(new_x, new_y):
                        new_matrix[new_x][new_y] = 1 - new_matrix[new_x][new_y]
...
if __name__ == "__main__":
    ...
    goal = tuple(1 for _ in range(len(fields)))
    board = BoardGame(fields, n, goal)

    game = BoardGame(fields, n, goal)
