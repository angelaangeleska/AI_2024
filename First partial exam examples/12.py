...
class Football(Problem):
    ...
    def successor(self, state):
        ...
        ball_x, ball_y = state[1][0], state[1][1]

        for move in moves:
            man_new_x, man_new_y = man_x + move[0], man_y + move[1]
            if (man_new_x, man_new_y) == (ball_x, ball_y):
                # turni topka
                new_ball_x, new_ball_y = ball_x + move[0], ball_y + move[1]
                if self.check_valid(man_new_x, man_new_y) and self.check_valid(new_ball_x, new_ball_y) and (new_ball_x, new_ball_y) not in self.illegal:
                    action = f'Turni topka {actions[moves.index(move)]}'
                    successors[action] = ((man_new_x, man_new_y), (new_ball_x, new_ball_y))
            else:
                # pomesti coveche
                if self.check_valid(man_new_x, man_new_y):
                    action = f'Pomesti coveche {actions[moves.index(move)]}'
                    successors[action] = ((man_new_x, man_new_y), (ball_x, ball_y))
