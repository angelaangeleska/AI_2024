...
class Squares(Problem):
    ...
    def successor(self, state):
        ...
        actions = ["gore", "dolu", "desno", "levo"]

        for i in range(5):
            squares = list(state)
            for move in moves:
                new_square = (state[i][0] + move[0], state[i][1] + move[1])
                squares[i] = new_square
                if self.check(new_square):
                    index = moves.index(move)
                    succ[f'Pomesti kvadratche {i+1} {actions[index]}'] = tuple(squares)
