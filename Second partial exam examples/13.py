...
def accuracy_calculator(test_x, test_y, classificator1, classificator2, classificator3):
    acc = 0
    for i in range(len(test_x)):
        a1, a2, a3 = 0, 0, 0
        predicted_class_1 = classificator1.predict([test_x[i]])[0]
        predicted_class_2 = classificator2.predict([test_x[i]])[0]
        predicted_class_3 = classificator3.predict([test_x[i]])[0]
        true_class = test_y[i]
        if true_class == predicted_class_1:
            a1 += 1
        if true_class == predicted_class_2:
            a2 += 1
        if true_class == predicted_class_3:
            a3 += 1
        if a1+a2+a3 >= 2:
            acc += 1
...
if __name__ == '__main__':
    ...
    train_set_nb = class_0[:int(len(class_0)*x1)] + class_1[:int(len(class_1)*x1)] + class_2[:int(len(class_2)*x1)]
    train_set_dt = class_0[int(len(class_0)*x1):int(len(class_0)*x2)] + class_1[int(len(class_1)*x1):int(len(class_1)*x2)] + class_2[int(len(class_2)*x1):int(len(class_2)*x2)]
    train_set_rf = class_0[:int(len(class_0)*x2)] + class_1[:int(len(class_1)*x2)] + class_2[:int(len(class_2)*x2)]

    train_x_nb, train_y_nb = [row[:-1] for row in train_set_nb], [row[-1] for row in train_set_nb]
    train_x_dt, train_y_dt = [row[:-1] for row in train_set_dt], [row[-1] for row in train_set_dt]
    train_x_rf, train_y_rf = [row[:-1] for row in train_set_rf], [row[-1] for row in train_set_rf]
    test_x, test_y = [row[:-1] for row in test_set], [row[-1] for row in test_set]

    classificator_nb = GaussianNB()
    classificator_nb.fit(train_x_nb, train_y_nb)
    classificator_dt = DecisionTreeClassifier(random_state=0)
    classificator_dt.fit(train_x_dt, train_y_dt)
    classificator_rf = RandomForestClassifier(3, random_state=0)
    classificator_rf.fit(train_x_rf, train_y_rf)

    acc = accuracy_calculator(test_x, test_y, classificator_nb, classificator_dt, classificator_rf)
