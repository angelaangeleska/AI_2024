...
def calculator(test_x, test_y, nb, dt, rf, mlp, index):
    nb_count, dt_count, rf_count, mlp_count = 0, 0, 0, 0
    tp, fn = 0, 0
    for i in range(len(test_x)):
        predicted_class_nb = nb.predict([test_x[i]])[0]
        if predicted_class_nb == test_y[i]:
            nb_count += 1

        predicted_class_dt = dt.predict([test_x[i]])[0]
        if predicted_class_dt == test_y[i]:
            dt_count += 1

        predicted_class_rf = rf.predict([test_x[i]])[0]
        if predicted_class_rf == test_y[i]:
            rf_count += 1

        predicted_class_mlp = mlp.predict([test_x[i]])[0]
        if predicted_class_mlp == test_y[i]:
            mlp_count += 1

        predictions = [nb_count, dt_count, rf_count, mlp_count]
        predictions[index] = predictions[index] * 2
        max_index = predictions.index(max(predictions))

        if max_index == 0:
            predicted_class = predicted_class_nb
        elif max_index == 1:
            predicted_class = predicted_class_dt
        elif max_index == 2:
            predicted_class = predicted_class_rf
        elif max_index == 3:
            predicted_class = predicted_class_mlp

        if predicted_class == test_y[i] and predicted_class == 1:
            tp += 1
        elif predicted_class != test_y[i] and predicted_class == 0:
            fn += 1
...
if __name__ == '__main__':
    ...
    train_set = negative_class[:int(len(negative_class)*percent)] + positive_class[:int(len(positive_class)*percent)]
    train_x, train_y = [row[:-1] for row in train_set], [row[-1] for row in train_set]

    test_set = negative_class[int(len(negative_class)*percent):] + positive_class[int(len(positive_class)*percent):]
    test_x, test_y = [row[:-1] for row in test_set], [row[-1] for row in test_set]

    classificator_nb = GaussianNB()
    classificator_nb.fit(train_x, train_y)

    classificator_dt = DecisionTreeClassifier(criterion='entropy', random_state=0)
    classificator_dt.fit(train_x, train_y)

    classificator_rf = RandomForestClassifier(4, criterion='entropy', random_state=0)
    classificator_rf.fit(train_x, train_y)

    classificator_mlp = MLPClassifier(10, activation='relu', learning_rate_init=0.001, random_state=0)
    classificator_mlp.fit(train_x, train_y)

    accuracy_nb = accuracy_score(test_y, classificator_nb.predict(test_x))
    accuracy_dt = accuracy_score(test_y, classificator_dt.predict(test_x))
    accuracy_rf = accuracy_score(test_y, classificator_rf.predict(test_x))
    accuracy_mlp = accuracy_score(test_y, classificator_mlp.predict(test_x))
    ...
    if max_index == 0:
        ...
        print("Najgolema tocnost klasifikatorot ima MLP")


    print(f"Odzivot na kolekcijata so klasifikatori e {calculator(test_x, test_y, classificator_nb, classificator_dt, classificator_rf, classificator_mlp, max_index)}")
