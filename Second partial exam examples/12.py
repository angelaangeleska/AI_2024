...
if __name__ == '__main__':
    ...
    classifier = MLPClassifier(n_neurons, activation='relu', max_iter=20, learning_rate_init=learning_rate, random_state=0)
    classifier.fit(scaler.transform(train_x), train_y)

    acc1 = accuracy_calculator(scaler.transform(train_x), train_y, classifier)
    acc2 = accuracy_calculator(scaler.transform(val_x), val_y, classifier)

    if acc1 > acc2 * 1.15:
        train_x_updated = [[row[i] for i in range(len(row)) if i != col_index] for row in train_x]
        scaler.fit(train_x_updated)
        classifier.fit(scaler.transform(train_x_updated), train_y)
